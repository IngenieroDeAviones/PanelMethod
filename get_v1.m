...
function [v1, r_pj1] = get_v1(panel_points, middle_points)
    ...
    for i=1:(length(panel_points)-1) %For all panels
        ...
        vectorPoint_x = middle_points(j,1) - panel_points(i+1,1); %X Coordinate Vector Point
        vectorPoint_y = middle_points(j,2) - panel_points(i+1,2); %Y Coordinate Vector Point
        ...
        v1(i,j) = angle; %Adding the angle to the matrix V0
        r_pj1(i,j) = norm(vectorPoint);
